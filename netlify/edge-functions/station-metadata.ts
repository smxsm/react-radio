...
const edge = async (req: Request) => {
  ...
  try {
    ...
    const icyHeaders = parseHeaders(res.headers);
    const data = await getIcecastMetadata(res, icyMetaInt);
    return new Response(JSON.stringify({ ...data, ...icyHeaders }), { headers: responseHeaders });
  } catch (err) {
    return new Response(JSON.stringify({}), { headers: responseHeaders });
  }
};

async function getIcecastMetadata(response: Response, icyMetaInt: number): Promise<{ title: string }> {
  return new Promise((resolve) => {
    new IcecastReadableStream(response, {
      onMetadata: ({ metadata }) => {
        resolve({ title: metadata.StreamTitle || metadata.TITLE });
      },
